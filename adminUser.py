...
def admin_display_menu():  # Function to display the admin menu
    ...
    action = input(
        "Would you like to book/cancel/reset or exit? (book/cancel/reset/exit)")
    if action == "book":  # If the action is to book a seat
        return admin_book_seat()  # Call the admin_book_seat function
    elif action == "cancel":  # If the action is to cancel a booking
        return admin_cancel_booking()  # Call the admin_cancel_booking function
    elif action == "reset":  # If the action is to reset the seating arrangement
...
def admin_book_seat():  # Function to book a seat by an admin
    ...
    row = int(
        input("Please enter the row number (1-10) of the seat you want to book:"))
    # Get the seat number of the seat to book
    seat = int(
        input("Please enter the seat number (1-10) of the seat you want to book:"))
    ...
    seating_arrangement[row - 1][1] = seating_arrangement[row -
                                                          1][1][:seat - 1] + "X" + seating_arrangement[row - 1][1][seat:]
...
def admin_cancel_booking():
    ...
    if (row, seat) in user_seats:
        ...
        seating_arrangement[row - 1][1] = seating_arrangement[row -
                                                              1][1][:seat - 1] + "-" + seating_arrangement[row - 1][1][seat:]
        save_seating_to_file(seating_arrangement)
